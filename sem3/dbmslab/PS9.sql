...
--1
BEGIN
for i in (select * into p from person) loop
  dbms_output.put_line(p.name || ' ' || p.age || ' ' || p.gender);
end loop;
END;

--2
declare
cursor c is select pizza, count(*) as coun from eats group by count(*) desc;
ct int := 1;
BEGIN
FOR i IN c loop
dbms_output.put_line(i.pizza || ' ' || i.coun);
ct := ct + 1;
EXIT when ct > 3;
END loop;
END;
